	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_DOWN: {
				firstMotionY = event.getY();
				
				System.out.println("firstMotionY--->" + firstMotionY);
			...
			case MotionEvent.ACTION_UP: {
				lastMotionY2 = event.getY();
				
				System.out.println("lastMotionY2--->" + lastMotionY2);
				
				if (isBeingDragged) {
					isBeingDragged = false;

					if (state == RELEASE_TO_REFRESH && null != onRefreshListener && (lastMotionY2 > firstMotionY)) {
